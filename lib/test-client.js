...
async function run() {
    ...
    const responder = new addon.LibEDHOC(20, 0, credentials, crypto);
    ...
    try {
        // Message 1 Composing
        let message1 = await initiator.composeMessage1([{ label: 9999, value: Buffer.from([0x01, 0x02, 0x03, 0x04]) }]);
        ...
        console.log(chalk.red("Message 2 - C_I"), initiator.connectionID);
        console.log(chalk.red("Message 2 - C_R"), initiator.peerConnectionID);
        ...
        console.log("OSCORE Context", await initiator.exportOSCORE());
