...
void EdhocCryptoManagerWrapper::SetGenerateKey(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameGenerateKey, this->manager->generateKeyFuncRef, this->manager->generateTsfn);
}

void EdhocCryptoManagerWrapper::SetDestroyKey(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameDestroyKey, this->manager->destroyKeyFuncRef, this->manager->destroyTsfn);
}

void EdhocCryptoManagerWrapper::SetMakeKeyPair(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameMakeKeyPair, this->manager->makeKeyPairFuncRef, this->manager->makeKeyPairTsfn);
}

void EdhocCryptoManagerWrapper::SetKeyAgreement(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameKeyAgreement, this->manager->keyAgreementFuncRef, this->manager->keyAgreementTsfn);
}

void EdhocCryptoManagerWrapper::SetSign(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameSign, this->manager->signFuncRef, this->manager->signTsfn);
}

void EdhocCryptoManagerWrapper::SetVerify(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameVerify, this->manager->verifyFuncRef, this->manager->verifyTsfn);
}

void EdhocCryptoManagerWrapper::SetExtract(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameExtract, this->manager->extractFuncRef, this->manager->extractTsfn);
}

void EdhocCryptoManagerWrapper::SetExpand(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameExpand, this->manager->expandFuncRef, this->manager->expandTsfn);
}

void EdhocCryptoManagerWrapper::SetEncrypt(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameEncrypt, this->manager->encryptFuncRef, this->manager->encryptTsfn);
}

void EdhocCryptoManagerWrapper::SetDecrypt(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameDecrypt, this->manager->decryptFuncRef, this->manager->decryptTsfn);
}

void EdhocCryptoManagerWrapper::SetHash(const Napi::CallbackInfo& info, const Napi::Value &value) {
    SetFunctionAndTsfn(value, kTsfnNameHash, this->manager->hashFuncRef, this->manager->hashTsfn);
}
...
Napi::Object EdhocCryptoManagerWrapper::Init(Napi::Env env, Napi::Object exports) {
    ...
    Napi::Function func = DefineClass(env, kClassName, {
        InstanceAccessor(kGenerateKeyAccessor, &EdhocCryptoManagerWrapper::GetGenerateKey, &EdhocCryptoManagerWrapper::SetGenerateKey),
        InstanceAccessor(kDestroyKeyAccessor, &EdhocCryptoManagerWrapper::GetDestroyKey, &EdhocCryptoManagerWrapper::SetDestroyKey),
        InstanceAccessor(kMakeKeyPairAccessor, &EdhocCryptoManagerWrapper::GetMakeKeyPair, &EdhocCryptoManagerWrapper::SetMakeKeyPair),
        InstanceAccessor(kKeyAgreementAccessor, &EdhocCryptoManagerWrapper::GetKeyAgreement, &EdhocCryptoManagerWrapper::SetKeyAgreement),
        InstanceAccessor(kSignAccessor, &EdhocCryptoManagerWrapper::GetSign, &EdhocCryptoManagerWrapper::SetSign),
        InstanceAccessor(kVerifyAccessor, &EdhocCryptoManagerWrapper::GetVerify, &EdhocCryptoManagerWrapper::SetVerify),
        InstanceAccessor(kExtractAccessor, &EdhocCryptoManagerWrapper::GetExtract, &EdhocCryptoManagerWrapper::SetExtract),
        InstanceAccessor(kExpandAccessor, &EdhocCryptoManagerWrapper::GetExpand, &EdhocCryptoManagerWrapper::SetExpand),
        InstanceAccessor(kEncryptAccessor, &EdhocCryptoManagerWrapper::GetEncrypt, &EdhocCryptoManagerWrapper::SetEncrypt),
        InstanceAccessor(kDecryptAccessor, &EdhocCryptoManagerWrapper::GetDecrypt, &EdhocCryptoManagerWrapper::SetDecrypt),
        InstanceAccessor(kHashAccessor, &EdhocCryptoManagerWrapper::GetHash, &EdhocCryptoManagerWrapper::SetHash)
    });
