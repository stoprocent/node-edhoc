...
void EdhocCryptoManagerWrapper::SetFunctionAndTsfn(
    const Napi::Value &value, const std::string &tsfnName,
    Napi::FunctionReference &functionRef, Napi::ThreadSafeFunction &tsfn) {
...
void EdhocCryptoManagerWrapper::SetGenerateKey(const Napi::CallbackInfo &info,
  ...
  SetFunctionAndTsfn(value, kTsfnNameGenerateKey,
                     this->manager->generateKeyFuncRef,
                     this->manager->generateTsfn);
}

void EdhocCryptoManagerWrapper::SetDestroyKey(const Napi::CallbackInfo &info,
                                              const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameDestroyKey,
                     this->manager->destroyKeyFuncRef,
                     this->manager->destroyTsfn);
}

void EdhocCryptoManagerWrapper::SetMakeKeyPair(const Napi::CallbackInfo &info,
                                               const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameMakeKeyPair,
                     this->manager->makeKeyPairFuncRef,
                     this->manager->makeKeyPairTsfn);
}

void EdhocCryptoManagerWrapper::SetKeyAgreement(const Napi::CallbackInfo &info,
                                                const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameKeyAgreement,
                     this->manager->keyAgreementFuncRef,
                     this->manager->keyAgreementTsfn);
}

void EdhocCryptoManagerWrapper::SetSign(const Napi::CallbackInfo &info,
                                        const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameSign, this->manager->signFuncRef,
                     this->manager->signTsfn);
}

void EdhocCryptoManagerWrapper::SetVerify(const Napi::CallbackInfo &info,
                                          const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameVerify, this->manager->verifyFuncRef,
                     this->manager->verifyTsfn);
}

void EdhocCryptoManagerWrapper::SetExtract(const Napi::CallbackInfo &info,
                                           const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameExtract, this->manager->extractFuncRef,
                     this->manager->extractTsfn);
}

void EdhocCryptoManagerWrapper::SetExpand(const Napi::CallbackInfo &info,
                                          const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameExpand, this->manager->expandFuncRef,
                     this->manager->expandTsfn);
}

void EdhocCryptoManagerWrapper::SetEncrypt(const Napi::CallbackInfo &info,
                                           const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameEncrypt, this->manager->encryptFuncRef,
                     this->manager->encryptTsfn);
}

void EdhocCryptoManagerWrapper::SetDecrypt(const Napi::CallbackInfo &info,
                                           const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameDecrypt, this->manager->decryptFuncRef,
                     this->manager->decryptTsfn);
}

void EdhocCryptoManagerWrapper::SetHash(const Napi::CallbackInfo &info,
                                        const Napi::Value &value) {
  SetFunctionAndTsfn(value, kTsfnNameHash, this->manager->hashFuncRef,
                     this->manager->hashTsfn);
}

Napi::Value
EdhocCryptoManagerWrapper::GetGenerateKey(const Napi::CallbackInfo &info) {
  return this->manager->generateKeyFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetDestroyKey(const Napi::CallbackInfo &info) {
  return this->manager->destroyKeyFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetMakeKeyPair(const Napi::CallbackInfo &info) {
  return this->manager->makeKeyPairFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetKeyAgreement(const Napi::CallbackInfo &info) {
...
Napi::Value
EdhocCryptoManagerWrapper::GetVerify(const Napi::CallbackInfo &info) {
  return this->manager->verifyFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetExtract(const Napi::CallbackInfo &info) {
  return this->manager->extractFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetExpand(const Napi::CallbackInfo &info) {
  return this->manager->expandFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetEncrypt(const Napi::CallbackInfo &info) {
  return this->manager->encryptFuncRef.Value();
}

Napi::Value
EdhocCryptoManagerWrapper::GetDecrypt(const Napi::CallbackInfo &info) {
...
Napi::Object EdhocCryptoManagerWrapper::Init(Napi::Env env,
  ...
  Napi::Function func = DefineClass(
      ...
      {InstanceAccessor(kGenerateKeyAccessor,
       ...
       InstanceAccessor(kSignAccessor, &EdhocCryptoManagerWrapper::GetSign,
                        &EdhocCryptoManagerWrapper::SetSign),
       InstanceAccessor(kVerifyAccessor, &EdhocCryptoManagerWrapper::GetVerify,
       ...
       InstanceAccessor(kExpandAccessor, &EdhocCryptoManagerWrapper::GetExpand,
       ...
       InstanceAccessor(kHashAccessor, &EdhocCryptoManagerWrapper::GetHash,
