 ...
 */
Napi::Object convert_edhoc_x5chain_to_js(const Napi::Env& env,
                                         const struct edhoc_auth_cred_x509_chain& x509_chain) {
  Napi::Object obj = Napi::Object::New(env);
 ...
 */
Napi::Object convert_edhoc_x5t_to_js(const Napi::Env& env,
...
int EdhocCredentialManager::FetchCredentials(void* user_context,
                                             struct edhoc_auth_creds* credentials) {
...
int EdhocCredentialManager::VerifyCredentials(void* user_context,
  ...
  UserContext* context = static_cast<UserContext*>(user_context);
  EdhocCredentialManager* manager = context->GetCredentialManager();
  return manager->callVerifyCredentials(context, credentials, public_key_reference,
                                        public_key_length);
}

int EdhocCredentialManager::callFetchCredentials(const void* user_context,
                                                 struct edhoc_auth_creds* credentials) {
  ...
  auto blockingCallHandler = [this, &user_context, &jsCallbackHandler, &errorHandler](
...
int EdhocCredentialManager::callVerifyCredentials(const void* user_context,
  ...
  auto blockingCallHandler = [this, &user_context, &credentials, &jsCallbackHandler, &errorHandler](
                                 Napi::Env env, Napi::Function jsCallback) {
    Napi::HandleScope scope(env);
