...
void EdhocCredentialManagerWrapper::SetFetch(const Napi::CallbackInfo& info,
                                             const Napi::Value& value) {
  SetFunctionAndTsfn(
      value, kFetchCredentials, manager->fetchFuncRef, manager->fetchTsfn);
}

void EdhocCredentialManagerWrapper::SetVerify(const Napi::CallbackInfo& info,
                                              const Napi::Value& value) {
  SetFunctionAndTsfn(
      value, kVerifyCredentials, manager->verifyFuncRef, manager->verifyTsfn);
}

Napi::Value EdhocCredentialManagerWrapper::GetFetch(
    const Napi::CallbackInfo& info) {
  return manager->fetchFuncRef.Value();
}

Napi::Value EdhocCredentialManagerWrapper::GetVerify(
    const Napi::CallbackInfo& info) {
