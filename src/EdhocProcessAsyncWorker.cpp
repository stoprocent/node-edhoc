    ...
    if (ret != EDHOC_SUCCESS) {
      ...
      switch (error_code) {
        case EDHOC_ERROR_CODE_WRONG_SELECTED_CIPHER_SUITE: {
          ...
          ret = edhoc_error_get_cipher_suites(&context, csuites, ARRAY_SIZE(csuites), &csuites_len,
                                              peer_csuites, ARRAY_SIZE(peer_csuites),
                                              &peer_csuites_len);
          if (ret == EDHOC_SUCCESS) {
            ...
            std::snprintf(errorMessage, kErrorBufferSize, kErrorWrongSelectedCipherSuiteFormat,
                          suites_str.c_str(), peer_suites_str.c_str());
      ...
      char errorMessage[kErrorBufferSize];
      std::snprintf(errorMessage, kErrorBufferSize, kErrorMessageFormat, messageNumber + 1,
                    error_code);
      SetError(errorMessage);
    }
