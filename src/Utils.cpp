...
void Utils::ResetAndRelease(Napi::FunctionReference& funcRef, Napi::ThreadSafeFunction& tsfn) {
  if (!funcRef.IsEmpty()) {
    funcRef.Reset();
  }
  if (tsfn != nullptr) {
...
void Utils::InvokeJSFunctionWithPromiseHandling(
  ...
  auto thenCallback = Napi::Function::New(env, [callbackLambda, errorLambda](const Napi::CallbackInfo& info) {
    Napi::Env env = info.Env();
    Napi::HandleScope scope(env);
    try {
      callbackLambda(env, info[0].As<Napi::Value>());
